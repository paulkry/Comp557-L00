      CSCE 441 – Lab 0 code{white-space: pre-wrap;} span.smallcaps{font-variant: small-caps;} span.underline{text-decoration: underline;} div.column{display: inline-block; vertical-align: top; width: 50%;} div.hanging-indent{margin-left: 1.5em; text-indent: -1.5em;} ul.task-list{list-style: none;}  
...
_IMPORTANT!_ In this course, your project must be cross-platform. Follow these guidelines.

*   Always use CMake to perform an [“out-of-source” build](http://voices.canonical.com/jussi.pakkanen/2013/04/16/why-you-should-consider-using-separate-build-directories/), which means that the binary files will not be in the same directory as the source files.
...
First, download the lab [here](L00.zip), and then extract the zip file to a suitable folder. Then go to the section corresponding to your development environment.

*   Windows
    *   [Install prerequisite software on Windows](#WIN)
    *   Option A: [Run the lab with Visual Studio](#VS)
    *   Option B: [Run the lab with Makefile](#MAKE)
*   macOS
    *   [Install prerequisite software on macOS](#MAC)
    *   Option A: [Run the lab with Xcode](#XCODE)
    *   Option B: [Run the lab with Makefile](#MAKE)
*   Linux
    *   [Install prerequisite software on Linux](#LINUX)
    *   [Run the lab with Makefile](#MAKE)
...
3.  Set up the environment variables. This ensures that your code will work on the TA’s and the instructor’s machines.
    ...
    *   Add the following lines to your “run commands” file (`.zshrc` or `.bashrc`).
        ...
        Set these variables to point to the directories that you extracted GLM, GLFW, and GLEW to. These directories should be the “root” directories of the libraries – i.e., they should contain such items as `include/`, `src/`, `doc/`, `lib/`, `readme`, etc. For example, on my machine, `ABS_PATH_TO_GLM` is `/Users/sueda/lib/glm-0.9.9.6`.
